...
public class QuotesGenerator {
    ...
    public static String generateRandomSentence(int length) {
        ...
        String filePath = "C:\\Users\\user\\Desktop\\fop\\FOP ASSIGNMENT SWING\\Quote.txt";
        try {
    ...
    public static String generateRandomQuotes(int length) {
        
        try {
            Scanner QuoteSearch = new Scanner(new FileInputStream("C:\\Users\\user\\Desktop\\fop\\FOP ASSIGNMENT SWING\\Quote.txt"));
			String protoS = "";
			while(QuoteSearch.hasNextLine()) 
			{protoS+=QuoteSearch.nextLine();}
			quotePool = protoS.split("/");
			Random random = new Random();
			randomQ = random.nextInt(0,quotePool.length);
        } catch (IOException e) {
            e.printStackTrace();
        }
